...
// Note: This version of the expression includes the market address above the table.
// These are fields all (or almost all) of my data wlll have, so I'm not doing checks.
// The address is displayed bigger and bolder than the table data.
// I ended up with an "alt" version (also in this repo) with address removed,
// since the address is already in the list, from which the popup info is accessed.
...
// Array to hold address components
var addr_arr = []

// Smish together fields composing address
Push(addr_arr, '<strong>' + $feature['StAddr'])
Push(addr_arr, $feature['City'] + ', ' + $feature['RegionAbbr'] + ' ' + $feature['Postal'] + '</strong>')

// Smash it all together with line break
var addr_text = Concatenate(addr_arr, '<br>')

// Set popup text equal to address text; will concat with table further down
...
// Smash together address and table into one long string
